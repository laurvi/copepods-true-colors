...
```{r, fig.height=6, fig.width=5, fig.align="center", fig.cap=c("Counts of studies that observed carotenoid pigmentation in different copepod genus.")}
# Most studied genus 
...
ggsave(filename="fig_sup/Fig.S1.png", width=11, height=12)
```
...
```{r fig.align="center", fig.cap=c("Figure S2. Carotenoid concentration (µg/mgDW, log-transformed) according to latitude (° in absolute value) in freshwater and marine ecosystems. Linear models are fitted but are not significant."), fig.height=3, fig.width=5}
ug  %>% 
  ggplot() + 
...
ggsave(filename="fig_sup/Fig.S2.png", width=6, height=3.5)
...
```{r fig.align="center", fig.cap=c("Figure S3. Carotenoid concentration (µg/mgDW, log-transformed) according to lake altitude (m) with a linear model fitted (significant, p-value < 0.001)."), fig.height=3, fig.width=5}
...
ggsave(filename="fig_sup/Fig.S3.png", width=6, height=4)
